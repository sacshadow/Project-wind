/*
	...
	notice that normals and tangets of the mesh was incurrect, you need fix it your self
	
	保存网格为 .obj文件
	注意obj模型的法线和切线不会正确导出，所以你需要自行修复
*/
...
public class ObjBuilder {
	...
	public static bool SaveAsObj(MeshFilter[] meshFilters, string name, string path, bool overrideFile){
		...
		for(i=0; i< meshFilters.Length; i++){
			...
			//获得材质球
			if(meshFilters[i].renderer){
				mats=meshFilters[i].renderer.sharedMaterials;
			}
			else{
				mats=new Material[0];
			}
